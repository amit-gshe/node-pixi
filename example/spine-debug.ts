import { SkeletonBounds } from "pixi-spine";
...
export class SpineDebug {
    ...
    for (let key in ghs) {
      if (ghs[key] instanceof Graphics) {
        ...
        for (let len = ghs[key].children.length; len > 0; len--) {
          ghs[key].children[len - 1].destroy({
            children: true,
            texture: true,
            baseTexture: true,
          });
  ...
  drawRegionAttachments() {
    ...
    for (let i = 0, len = slots.length; i < len; i++) {
      ...
      let vertices: any = new Float32Array(8);
  ...
  drawBoundingBoxes() {
    ...
    ghs.boundingBoxesRect.drawRect(
      bounds.minX,
      bounds.minY,
      bounds.getWidth(),
      bounds.getHeight()
